...
def display_weather_data(canva):
    ...
    canva.add_object(Text(text_font, X+410, Y+40, "m: {}°C".format(weatherAPI.tomorrow_min_temperature), 0, "left"))
    canva.add_object(Text(text_font, X+410, Y+65, "M: {}°C".format(weatherAPI.tomorrow_max_temperature), 0, "left"))
...
def display_title_date(canva):
    ...
    canva.add_object(Text(title_font, X_size//2 - (text_width / 2), Y_size//2 - (text_height / 2)-5, "Bonjour Lisa", 0, "left"))
    
    
def display_footer(canva):
    canva.add_object(Rectangle(0,772,479,35,0))
...
def display_todolist(canva):
    ...
    for i in range(len(tasks)): 
        element = tasks[i]
        canva.add_object(Text(chalk_font, X_corner + (i%2)*X_spacing + rd.randint(-3,3), Y_corner + (i//2)*Y_spacing + rd.randint(-3,3), "- " + element, 0, "left"))
